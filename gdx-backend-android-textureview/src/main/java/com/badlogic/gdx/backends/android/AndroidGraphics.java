 ...
 * @author mzechner
 */
public class AndroidGraphics extends AbstractGraphics implements Renderer, GLTextureView.Renderer {
    ...
    protected void preserveEGLContextOnPause() {
        if (view instanceof GLSurfaceView) {
            ...
            ((GLTextureView) view).setPreserveEGLContextOnPause(true);
        }
    }
    ...
    void pause() {
        synchronized (synch) {
            ...
            if (view instanceof GLSurfaceView) {
                ((GLSurfaceView) view).queueEvent(new Runnable() {
                    @Override
                    public void run() {
                        if (!pause) {
                ...
                ((GLTextureView) view).queueEvent(new Runnable() {
                    @Override
                    public void run() {
                        if (!pause) {
                            // pause event already picked up by onDrawFrame
